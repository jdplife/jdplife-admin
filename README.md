...
**演示地址：** [https://jdplifeadmin.cn/](https://jdplifeadmin.cn/)

**【火星编程导航】学编程做项目：** [https://jdplife-coder.cn/](https://jdplife-coder.cn/)
...
1. **克隆项目**
```bash
git clone https://gitee.com/jdplifefactory/jdplife-admin.git
cd jdplife-admin
```
...
```sql
-- 创建数据库
CREATE DATABASE jdplife-system DEFAULT CHARACTER SET utf8mb4;

-- 导入 SQL
mysql -u root -p jdplife-system < sql/jdplife-system.sql
```

3. **修改配置**

修改 `jdplife-starter/src/main/resources/application-dev.yml` 中的数据库和 Redis 配置：

```yaml
spring:
  datasource:
    url: jdbc:mysql://localhost:3306/jdplife-system
    username: root
...
使用 HBuilderX 打开 `jdplife-uniapp` 目录，运行到微信开发者工具即可。
...
1. 在 `jdplife-core/jdplife-system/entity` 下创建实体类
2. 在 `jdplife-core/jdplife-system/mapper` 下创建 Mapper 接口
3. 在 `jdplife-core/jdplife-system/service` 下创建 Service 接口和实现
4. 在 `jdplife-api/jdplife-admin-api/controller` 下创建 Controller
...
在 `jdplife-core/jdplife-auth/strategy` 下创建新的登录策略类，实现 `LoginStrategy` 接口：
